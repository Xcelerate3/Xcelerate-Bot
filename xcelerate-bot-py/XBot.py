...
def get_password():
    ...
    if 'ENCRYPTPRIVATEKEYS' not in settings:
        ...
        while response != "y" and response != "n":
            ...
            response = input("You will need to input this password each time Xcelerate Bot is executed (y/n): ")
    ...
    if actual_allowance < allowance_to_compare_with or actual_allowance == 0:
        if settings["EXCHANGE"] == 'quickswap':
            ...
            if condition == 'base_approve':
                printt_info("---------------------------------------------------------------------------------------------")
                printt_info("Your base token is not approved --> Xcelerate Bot will now APPROVE it, or it won't be able to buy")
                printt_info("---------------------------------------------------------------------------------------------")
            elif condition == 'preapprove':
                printt_info("-----------------------------------------------------------------------------")
                printt_info("You have selected PREAPPROVE = true --> Xcelerate Bot will now APPROVE this token")
                printt_info("-----------------------------------------------------------------------------")
            elif condition == 'txfail':
                printt_info("----------------------------------------------------------------------------------")
                printt_info("You have failed to sell tokens --> Xcelerate Bot will chack if it needs to be APPROVED")
                printt_info("----------------------------------------------------------------------------------")
            else:
                printt_info("-------------------------------------")
                printt_info("Xcelerate Bot will now APPROVE this token")
                printt_info("-------------------------------------")

            tx = approve(address, allowance_request)
            wait_for_tx(token, tx, address)
            printt_ok("---------------------------------------------------------")
            printt_ok("  Token is now approved : Xcelerate Bot can sell this token", write_to_log=True)
            printt_ok("---------------------------------------------------------")
        
        return allowance_request
    
    else:
        printt_ok("Token is already approved --> Xcelerate Bot can use this token ")
...
def calculate_gas(token):
    ...
    if int(token['GASLIMIT']) < 250000:
        printt_info(
            "Your GASLIMIT parameter is too low : Xclerate Bot has forced it to 300000 otherwise your transaction would fail for sure. We advise you to raise it to 1000000.")
...
def preapprove_base(token):
    ...
    printt("Xcelerate Bot will now check approval of your Base or Custom Base for", token['SYMBOL'], ", to ensure you can use it")
...
def buy(token_dict, inToken, outToken, pwd):
    ...
    if int(gaslimit) < 250000:
        printt_info("Your GASLIMIT parameter is too low : Xcelerate Bot has forced it to 300000 otherwise your transaction would fail for sure. We advise you to raise it to 1000000.")
        gaslimit = 300000
...
def sell(token_dict, inToken, outToken):
    ...
    if int(gaslimit) < 250000:
        gaslimit = 300000
        printt_info(
            "Your GASLIMIT parameter is too low: Xcelerate has forced it to 300000 otherwise your transaction would fail for sure. We advise you to raise it to 1000000.")
    ...
    if balance >= 1:
        ...
        printt("Your", token_dict['SYMBOL'], "balance is very small (< 1), so Xcelerate Bot will not try to sell, so as not to waste fees.")
        return False
                    ...
                    if price_conditions_met == True:
                        ...
                        if tx != False:
                            ...
                            if txsellresult != 1:
                                ...
                                printt_err("Type of failures and possible causes:")
                                printt_err("- TRANSFER_FROM_FAILED         --> GASLIMIT too low. Raise it to GASLIMIT = 1000000 at least")
                                printt_err("- TRANSFER_FROM_FAILED         --> Token is not approved for trade. Xcelerate Bot will check approval right now.")
                                printt_err("- INSUFFICIENT_OUTPUT_AMOUNT   --> SLIPPAGE too low")
